...
class MyScene extends THREE.Scene {
  ...
  constructor(myCanvas) {
    ...
    this.mesh={
      "Hierba": new THREE.InstancedMesh(this.h.geometria, this.h.material, 2 * this.TAM_CHUNK * this.TAM_CHUNK * this.DISTANCIA_RENDER * this.DISTANCIA_RENDER),
      "Piedra": new THREE.InstancedMesh(this.p.geometria, this.p.material, 7 * this.TAM_CHUNK * this.TAM_CHUNK * this.DISTANCIA_RENDER * this.DISTANCIA_RENDER),
      ...
      for (let j = 0; j < this.DISTANCIA_RENDER; j++) {
        ...
        for (let x = i * this.TAM_CHUNK; x < (i * this.TAM_CHUNK) + this.TAM_CHUNK; x++) {   //PARA GENERAR LOS BLOQUES DE UN CHUNK
          for (let z = j * this.TAM_CHUNK; z < (j * this.TAM_CHUNK) + this.TAM_CHUNK; z++) {
            ...
            k++;
            for(let r = v - 8/PM.PIXELES_ESTANDAR - 1; r > -14; r--){
              matrix.setPosition(x * 16 / PM.PIXELES_ESTANDAR, r , z * 16 / PM.PIXELES_ESTANDAR);
              this.mesh["Piedra"].setMatrixAt(contador, matrix);
  
              
              bloques.push({ x: x * 16 / PM.PIXELES_ESTANDAR, y: r, z: z * 16 / PM.PIXELES_ESTANDAR, material: "Piedra" });
              contador++;
            }
          }
        }
  ...
  onDocumentMouseDown(event) {
    ...
    if (event.which == 3) {
      ...
      for(let a in this.mesh){
        let objetos = raycaster.intersectObject(this.mesh[a], true);
        ...
        if (objetos[0] != undefined && objetos.length > 0 && objetos[0].distance <= 20) {
          ...
          objetosIntersecados.push({tipo: a, coordenada: coord, distancia: objetos[0].distance});
      ...
      this.chunk[aux.x][aux.z].push( {tipo: objetosIntersecados[0].tipo, coordenada: objetosIntersecados[0].coordenada} );
